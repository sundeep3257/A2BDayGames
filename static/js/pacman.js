...
(function() {
    ...
    function checkCollisions() {
        ...
        // Player vs Food
        // Check collision with player's yellow outline square (more lenient - any part of outline touches food)
        const cellSize = getCellSize();
        const playerSize = cellSize * 0.8;
        const playerOutlineWidth = 6; // Yellow outline width
        const playerTotalSize = playerSize + playerOutlineWidth; // Total size including outline
        
        food = food.filter(f => {
            const foodPos = { x: f.x + 0.5, y: f.y + 0.5 };
            const foodX = foodPos.x * cellSize;
            const foodY = foodPos.y * cellSize;
            const foodRadius = 6; // Food pellet radius
            
            // Player bounds including yellow outline
            const playerX = player.x * cellSize;
            const playerY = player.y * cellSize;
            const playerLeft = playerX - playerTotalSize / 2;
            const playerRight = playerX + playerTotalSize / 2;
            const playerTop = playerY - playerTotalSize / 2;
            const playerBottom = playerY + playerTotalSize / 2;
            
            // Check if food circle intersects with player's square (including outline)
            const closestX = Math.max(playerLeft, Math.min(foodX, playerRight));
            const closestY = Math.max(playerTop, Math.min(foodY, playerBottom));
            const distX = foodX - closestX;
            const distY = foodY - closestY;
